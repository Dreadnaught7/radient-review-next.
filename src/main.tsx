...
async function fetchReports(limit = 100, slug?: string) {
  const params = new URLSearchParams({ select: REPORT_FIELDS, status: 'eq.published', is_public: 'eq.true', order: 'sort_rank.asc.nullslast,published_at.desc', limit: String(limit) });
...
function ReportView({ slug, preview }: { slug: string; preview?: Post }) {
  ...
  useEffect(() => { let active = true; if (preview?.content) { setPost(preview); return () => { active = false; }; } fetchReports(1, slug).then(items => active && setPost(items[0])).catch(() => {}); return () => { active = false; }; }, [slug, preview?.content]);
...
function App() {
  const [posts, setPosts] = useState<Post[]>([]); const [loading, setLoading] = useState(true); const [route, setRoute] = useState(window.location.hash || '#/'); const [searchOpen, setSearchOpen] = useState(false);
  useEffect(() => { fetchReports(100).then(setPosts).catch(() => setPosts([])).finally(() => setLoading(false)); }, []);
  ...
  if (!path) view = <Home posts={posts.slice(0,11)}/>; else if (path === 'archive') view = <Archive posts={posts} loading={loading}/>; else if (path === 'topics') view = <Topics/>; else if (path.startsWith('topic/')) view = <Archive posts={posts} filter={decodeURIComponent(path.slice(6))} loading={loading}/>; else if (path.startsWith('report/')) { const slug = decodeURIComponent(path.slice(7)); view = <ReportView slug={slug} preview={posts.find(post => post.slug === slug)}/>; } else if (path.startsWith('page/')) view = <PageView slug={decodeURIComponent(path.slice(5))}/>; else view = <Archive posts={posts} loading={loading}/>;
