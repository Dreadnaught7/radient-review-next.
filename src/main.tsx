...
const fixedImages: Record<string, ImageRef> = {
  ...
  'campus-research-gap-independent-research-universities': { src: 'https://commons.wikimedia.org/wiki/Special:Redirect/file/Reading_room_in_Rush_Rhees_library_at_the_University_of_Rochester.jpg?width=1400', credit: 'Rush Rhees Library, University of Rochester · Wikimedia Commons' },
};
...
function Archive({ posts, filter, loading }: { posts: Post[]; filter?: string; loading?: boolean }) {
  ...
  useEffect(() => setVisibleCount(18), [filter]);
  ...
  return <section className="interior shell"><a className="back" href="#/">← Home</a><div className="page-head"><p className="eyebrow">REPORT ARCHIVE</p><h1>{filter ? visible[0]?.category || 'Topic reports' : 'The complete current file'}</h1><p>{visible.length} reports.</p></div><div className="archive-grid">{shown.map(post => <ReportCard key={post.slug} post={post}/>)}</div>{visibleCount < visible.length && <div style={{display:'flex',justifyContent:'center',padding:'28px 0 8px'}}><button className="secondary" onClick={() => setVisibleCount(count => count + 18)}>Load more reports</button></div>}</section>;
}
