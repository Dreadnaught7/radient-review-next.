...
async function resolveImage(post: Post): Promise<ImageRef | null> {
  ...
  if (cached) {
    ...
    claimed.add(parsed.src);
    return parsed;
  }
  try {
    const query = encodeURIComponent(searchTerms(post) || post.category);
    const url = `https://commons.wikimedia.org/w/api.php?action=query&generator=search&gsrsearch=${query}&gsrnamespace=6&gsrlimit=12&prop=imageinfo&iiprop=url&iiurlwidth=1200&format=json&origin=*`;
    ...
    sessionStorage.setItem(`rr-img-${post.slug}`, JSON.stringify(result));
    return result;
  } catch {
    return null;
  }
...
function ArticleImage({ post, priority = false, compact = false }: { post: Post; priority?: boolean; compact?: boolean }) {
  ...
  useEffect(() => {
    let active = true;
    resolveImage(post).then(result => active && result && setImage(result));
    return () => { active = false; };
  }, [post.slug]);
  return <div className={`media ${compact ? 'media-compact' : ''}`}>
    {image ? <img src={image.src} alt="" loading={priority ? 'eager' : 'lazy'} decoding="async" /> : <div className={`signal signal-${hash(post.slug) % 4}`} />}
    <span>{post.category}</span>
    {image && <small>{image.credit}</small>}
  </div>;
}

function Brand() {
  return <a className="brand" href="#/"><span className="brand-mark">R</span><span><b>The Radient Review</b><small>FIND THE SIGNAL. KEEP THE COMPLEXITY.</small></span></a>;
}

function Header({ onSearch }: { onSearch: () => void }) {
  const [open, setOpen] = useState(false);
  return <header className="site-header"><div className="shell header-inner"><Brand/><nav className={open ? 'nav-open' : ''}>
    <a href="#/archive" onClick={() => setOpen(false)}>Reports</a><a href="#/topics" onClick={() => setOpen(false)}>Topics</a><a href="#/page/research-services" onClick={() => setOpen(false)}>Research</a><a href="#/page/about" onClick={() => setOpen(false)}>About</a><a href="#/page/support" onClick={() => setOpen(false)}>Support</a>
  </nav><div className="header-actions"><button className="icon-button" aria-label="Search" onClick={onSearch}><Search size={17}/></button><a className="subscribe" href="#/page/support">Subscribe</a><button className="menu-button" aria-label="Menu" onClick={() => setOpen(v => !v)}>{open ? <X size={20}/> : <Menu size={20}/>}</button></div></div></header>;
}

function SectionHead({ eyebrow, title, link, label }: { eyebrow: string; title: string; link?: string; label?: string }) {
  return <div className="section-head"><div><p className="eyebrow">{eyebrow}</p><h2>{title}</h2></div>{link && <a href={link}>{label || 'View all'} <ArrowRight size={15}/></a>}</div>;
}

function ReportCard({ post, compact = false }: { post: Post; compact?: boolean }) {
  return <article className={`report-card ${compact ? 'compact-card' : ''}`}><a href={`#/report/${post.slug}`}><ArticleImage post={post} compact={compact}/><div className="card-copy"><p className="eyebrow accent">{post.category}</p><h3>{post.title}</h3>{!compact && <p>{post.excerpt}</p>}<span>Read report <ArrowRight size={14}/></span></div></a></article>;
}
...
function Home({ posts }: { posts: Post[] }) {
  ...
  return <>
    <section className="hero shell"><div className="hero-copy"><p className="eyebrow">INDEPENDENT · CLEAR CONTEXT · OPEN QUESTIONS</p><h1>Find the signal.<em>Keep the complexity.</em></h1><p className="lede">Independent reports on technology, culture, markets, public systems and the patterns connecting them. Clear context without flattening the hard parts.</p><div className="hero-actions"><a className="primary" href="#/archive">Explore reports <ArrowRight size={15}/></a><a className="secondary" href="#/page/research-services">How we work</a></div></div><a className="hero-feature" href={`#/report/${lead.slug}`}><ArticleImage post={lead} priority/><div><p className="eyebrow accent">LATEST SIGNAL</p><h2>{lead.title}</h2></div></a></section>

    <section className="method-band"><div className="shell method-grid"><div><span>01</span><b>Signal</b></div><div><span>02</span><b>Evidence</b></div><div><span>03</span><b>Dissonance</b></div><div><span>04</span><b>Alignment</b></div><div><span>05</span><b>Resonance</b></div></div></section>

    <section className="section shell"><SectionHead eyebrow="FEATURED REPORT" title="Independent analysis, built to hold up." link="#/archive" label="Latest reports"/><div className="featured-layout"><ArticleImage post={lead} priority/><div className="featured-copy"><p className="eyebrow accent">{lead.category}</p><h2>{lead.title}</h2><p>{lead.excerpt}</p><a href={`#/report/${lead.slug}`}>Read the featured report <ArrowRight size={15}/></a></div></div></section>

    <section className="section tinted"><div className="shell"><SectionHead eyebrow="TRENDING NOW" title="Signals worth following."/><div className="trending-grid">{posts.slice(0,4).map(post => <ReportCard key={post.slug} post={post} compact/>)}</div></div></section>

    <section className="section shell"><SectionHead eyebrow="EXPLORE TOPICS" title="Different domains. Same standard of evidence."/><div className="topic-grid"><a href="#/topic/technology-infrastructure"><b>Technology</b><span>AI, infrastructure, platforms and systems</span></a><a href="#/topic/money-business"><b>Markets</b><span>Money, incentives, business and risk</span></a><a href="#/topic/media-culture"><b>Culture</b><span>Media, sports, behavior and identity</span></a><a href="#/topic/systems-everyday-life"><b>Public Systems</b><span>Policy, institutions and operations</span></a></div></section>

    <section className="section shell"><SectionHead eyebrow="LATEST REPORTS" title="The current file." link="#/archive" label="Browse archive"/><div className="latest-grid">{posts.slice(0,6).map(post => <ReportCard key={post.slug} post={post}/>)}</div></section>

    <section className="research"><div className="shell research-grid"><div><p className="eyebrow">RESEARCH & DECISION SUPPORT</p><h2>Your data.<br/>Your decisions.<em>Your rights.</em></h2><p>Research for people who need to understand what the evidence supports, what it does not, and what deserves another question.</p><a className="secondary gold" href="#/page/research-services">Research services</a></div><div className="principles"><div><span>01</span><h3>Evidence & lineage</h3><p>Keep sources distinguishable from interpretation.</p></div><div><span>02</span><h3>Competing explanations</h3><p>Do not collapse ambiguity for convenience.</p></div><div><span>03</span><h3>Decision clarity</h3><p>Show what is known, inferred, uncertain and changeable.</p></div></div></div></section>
  </>;
}

function Archive({ posts, filter }: { posts: Post[]; filter?: string }) {
...
function ReportView({ post }: { post?: Post }) {
...
function PageView({ slug }: { slug: string }) {
  ...
  useEffect(() => {
    setPage(null); setFailed(false);
    fetch(`${API}/pages?slug=${encodeURIComponent(slug)}`).then(r => r.ok ? r.json() : Promise.reject()).then(items => {
      if (!items[0]) throw new Error();
      setPage({ title: cleanText(items[0].title?.rendered), content: cleanHtml(items[0].content?.rendered) });
    }).catch(() => setFailed(true));
  }, [slug]);
  if (failed) return <section className="interior shell"><h1>Page unavailable.</h1></section>;
...
function SearchModal({ posts, onClose }: { posts: Post[]; onClose: () => void }) {
  ...
  const results = useMemo(() => {
    ...
    return q ? posts.filter(post => `${post.title} ${post.excerpt} ${post.category}`.toLowerCase().includes(q)).slice(0,8) : [];
  }, [query, posts]);
...
function Footer() {
  return <footer><div className="shell footer-grid"><div><Brand/><p>Your data. Your decisions. Your rights.</p></div><div><a href="#/archive">Reports</a><a href="#/page/about">About</a><a href="#/page/request-a-report">Request a report</a><a href="#/page/submit">Feedback</a><a href="#/page/support">Support</a></div></div></footer>;
}
...
function App() {
  ...
  useEffect(() => {
    fetch(`${API}/posts?per_page=100&_embed=1`).then(r => r.ok ? r.json() : Promise.reject()).then(items => setPosts(items.map(toPost))).catch(() => setPosts([]));
  }, []);
  useEffect(() => {
  ...
  if (!path) view = <Home posts={posts}/>;
  else if (path === 'archive') view = <Archive posts={posts}/>;
  else if (path === 'topics') view = <Topics/>;
  else if (path.startsWith('topic/')) view = <Archive posts={posts} filter={decodeURIComponent(path.slice(6))}/>;
  else if (path.startsWith('report/')) view = <ReportView post={posts.find(post => post.slug === decodeURIComponent(path.slice(7)))}/>;
  else if (path.startsWith('page/')) view = <PageView slug={decodeURIComponent(path.slice(5))}/>;
  else view = <Archive posts={posts}/>;
